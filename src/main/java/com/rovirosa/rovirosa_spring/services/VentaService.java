...
public class VentaService {
        ...
        public RutaDetalleResponseDTO getVentaById(Integer ventaId) {
                ...
                List<DetalleVentaResponseDTO> detalles = detalleVentaRep.findByVenta_Id(
                                venta.getVenta_Id());
                RutaDetalleResponseDTO rutaRes = new RutaDetalleResponseDTO(venta);
        ...
        public ApiResponse<Void> entregarVenta(Integer ventaId) {
                Integer result = ventaRep.updateEstadoVentaToEntregado(
                                ventaId,
                ...
                Venta venta = ventaRep.findById(ventaId).orElse(null);
                if (venta != null)
                        notificacionService.create(venta.getId(), "Venta entregada",
        ...
        public List<VentaResponseClienteDTO> getVentasByClienteId(Integer clienteId) {
                ...
                return ventas
                                .stream()
                                .map(venta -> new VentaResponseClienteDTO(
                                                venta,
        ...
        private void crearRutaVenta(VentaPostDTO ventaDTO, Venta venta) {
                System.out.println(
                                "\n\n============================================================================");
                System.out.println(
                                AZUL + "Iniciando proceso de asignación de ruta y repartidor..." + RESET);

                // Buscar repartidor en espera y cargando para el punto de venta
                List<Repartidor> repEnEspera = repartidorAsignRep.findRepartidoresActivos(
                                ventaDTO.getPvId(),
                                List.of("en_espera"));
                List<Repartidor> repCargando = repartidorAsignRep.findRepartidoresActivos(
                                ventaDTO.getPvId(),
                                List.of("cargando"));
                ...
                if (repEnEspera.isEmpty() && repCargando.isEmpty()) {
                        System.out.println(
                                        AMARILLO +
                                                        "No hay repartidores disponibles, buscando ruta cercana para juntarlos..."
                                                        +
                                                        RESET);
                        List<RutaDetalleQueryByRepartidorIdDTO> rutaDetalles = rutaDetalleRepository
                                        .findByRuta_PuntoVenta_IdAndRuta_EstadoAndRuta_RepartidorIsNull(
                                                        ventaDTO.getPvId(),
                                                        "pendiente");
                        if (!rutaDetalles.isEmpty()) {
                                List<RepartidorGetVehDTO> repartidores = repartidorRep
                                                .findByUsuario_PuntoVenta_Id(ventaDTO.getPvId());

                                for (RepartidorGetVehDTO repartidor : repartidores) {
                                        // Verificar que el repartidor tenga vehiculo asignado
                                        if (repartidor.getVehiculo() == null) {
                                                System.out.println(
                                                                ROJO +
                                                                                "Repartidor no tiene vehiculo asignado, saltando..."
                                                                                +
                                                                                RESET);
                                                continue;
                                        }
                                        // Si tiene vehiculo, verificar que tenga espacio disponible
                                        BigDecimal capacidadKg_Vehiculo = repartidor
                                                        .getVehiculo()
                                                        .getCapacidadKg();
                                        BigDecimal volumenM3_Vehiculo = repartidor
                                                        .getVehiculo()
                                                        .getVolumenM3();
                                        capacidadKg_Vehiculo = capacidadKg_Vehiculo.multiply(
                                                        BigDecimal.valueOf(repartidor.getVehiculo().getFactorUsoMax()));
                                        volumenM3_Vehiculo = volumenM3_Vehiculo.multiply(
                                                        BigDecimal.valueOf(repartidor.getVehiculo().getFactorUsoMax()));

                                        System.out.println(
                                                        AZUL +
                                                                        "[VEHICULO]: Capacidad Kg: " +
                                                                        capacidadKg_Vehiculo +
                                                                        " Volumen M3: " +
                                                                        volumenM3_Vehiculo +
                                                                        " Factor de uso MAX: " +
                                                                        repartidor.getVehiculo().getFactorUsoMax() +
                                                                        " - Cantidades ya con el factor de uso aplicado"
                                                                        +
                                                                        RESET);

                                        for (RutaDetalleQueryByRepartidorIdDTO rutaDet : rutaDetalles) {
                                                ...
                                                if (this.puedeEntregar(
                                                        ...
                                                        System.out.println(
                                                                        AZUL +
                                                                                        "Ruta encontrada cercana en el POINT ID: "
                                                                                        +
                                                                                        rutaDet.getId() +
                                                                                        RESET);
                                                        // Crear ruta sin repartidor
                                                        Ruta nuevaRuta = new Ruta();
                                                        nuevaRuta.setId(rutaDet.getRuta_Id());
                                                        ...
                                                        System.out.println(
                                                                        AZUL +
                                                                                        "El pedido se junto con otro para optimizar espacio."
                                                                                        +
                                                                                        RESET);
                                                        System.out.println(
                                                                        "============================================================================\n\n");
                        ...
                        System.out.println(
                                        AMARILLO +
                                                        "No se encontró ruta cercana, creando nueva ruta sin repartidor."
                                                        +
                                                        RESET);
                        // Crear ruta sin repartidor
                        Ruta nuevaRuta = new Ruta();
                        ...
                        System.out.println(
                                        AZUL +
                                                        "Proceso de asignación de ruta y repartidor finalizado." +
                                                        RESET);
                        System.out.println(
                                        "============================================================================\n\n");
                ...
                System.out.println(
                                VERDE + "Repartidores disponibles encontrados." + RESET);
                 ...
                 */
                System.out.println(
                                AMARILLO + "Buscando entre los repartidores 'cargando'..." + RESET);
                for (Repartidor repartidor : repCargando) {
                        System.out.println(
                                        AZUL +
                                                        "Evaluando repartidor cargando: " +
                        ...
                        if (repartidor.getVehiculo() == null) {
                                System.out.println(
                                                ROJO + "Repartidor no tiene vehiculo asignado, saltando..." + RESET);
                                continue;
                        }
                        ...
                        List<RutaDetalleQueryByRepartidorIdDTO> rutasActivas = rutaDetalleRepository
                                        .findByRuta_Repartidor_IdAndRuta_Estado(
                                                        repartidor.getId(),
                                                        "pendiente");
                        ...
                        System.out.println(
                                        AMARILLO +
                                                        "Evaluando POINTS en las rutas activas del repartidor ID: " +
                                                        repartidor.getId() +
                                                        " Cantidad de POINTS en rutas activas: " +
                                                        rutasActivas.size() +
                                                        RESET);

                        // Recorrer todos los puntos de entrega en la ruta del repartidor
                        for (RutaDetalleQueryByRepartidorIdDTO rutaAct : rutasActivas) {
                                ...
                                System.out.println(
                                                CIAN +
                                                                "Sumando dimensiones de venta ID: " +
                                                                rutaAct.getVenta_Id() +
                                ...
                                System.out.println(
                                                AZUL +
                                                                "Dimensiones acumuladas hasta ahora - Peso Kg: " +
                                                                pesoTotal +
                                                                " Volumen M3: " +
                                                                volTotal +
                                                                RESET);
                        ...
                        System.out.println(
                                        AZUL +
                                                        "Dimensiones totales en ruta - Peso Kg: " +
                                                        pesoTotal +
                                                        " Volumen M3: " +
                                                        volTotal +
                                                        RESET);
                        ...
                        if (estaCerca && rutaId != -1) {
                                ...
                                System.out.println(
                                                AZUL +
                                                                "Dimensiones pedido - Peso Kg: " +
                                                                dim.getPesoKg() +
                                                                " Volumen M3: " +
                                                                dim.getVolM3() +
                                                                RESET);

                                BigDecimal pesoTotalPedido = dim
                                                .getPesoKg()
                                                .add(BigDecimal.valueOf(pesoTotal));
                                BigDecimal volumenTotalPedido = dim
                                                .getVolM3()
                                                .add(BigDecimal.valueOf(volTotal));

                                System.out.println(
                                                AZUL +
                                                                "Dimensiones totales con nuevo pedido - Peso Kg: " +
                                                                pesoTotalPedido +
                                                                " Volumen M3: " +
                                                                volumenTotalPedido +
                                                                RESET);
                                ...
                                if (pesoTotalPedido.compareTo(capacidadKg_Vehiculo) < 0 &&
                                        ...
                                        System.out.println(
                                                        VERDE +
                                                                        "Asignando al repartidor cargando ID: " +
                                                                        repartidor.getId() +
                                                                        RESET);
                                        System.out.println(
                                                        VERDE + "Ruta seleccionada ID: " + rutaId + RESET);
                                        System.out.println(
                                                        VERDE +
                                        ...
                                        RutaDetalleResponseDTO rutaRes = new RutaDetalleResponseDTO(
                                                        rutaDetalle);
                                        ...
                                        System.out.println(
                                                        CIAN +
                                                                        "Enviando notificación por WebSocket al repartidor ID: "
                                                                        +
                                                                        repartidor.getId() +
                                                                        RESET);
                                        template.convertAndSend(
                                                        "/topic/nuevos-pedidos/" + repartidor.getId(),
                                                        rutaRes);

                                        // Enviar notificación al repartidor
                                        this.notificacionService.create(repartidor.getUsuario().getId(),
                                ...
                                System.out.println(
                                                AZUL +
                                                                "Dimensiones del pedido exceden la capacidad del vehiculo."
                                                                +
                                                                RESET);
                        }
                        System.out.println(
                                        ROJO + "Repartidor no esta cerca, saltando..." + RESET);
                }

                System.out.println(
                                ROJO +
                                                "No se pudo asignar a ningun repartidor cargando, buscando entre los en espera."
                                                +
                                                RESET);
                ...
                for (Repartidor repartidor : repEnEspera) {
                        System.out.println(
                                        AMARILLO +
                                                        "Evaluando repartidor en espera: " +
                                                        repartidor.getUsuario().getPersona().getNombre() +
                                                        RESET);
                        ...
                        if (pesoPedido.compareTo(capacidadKg_Vehiculo) < 0 &&
                                ...
                                System.out.println(
                                                VERDE +
                                                                "Asignando al repartidor en espera ID: " +
                                                                repartidor.getId() +
                                                                RESET);
                                ...
                                System.out.println(
                                                VERDE + "Ruta creada ID: " + nuevaRuta.getId() + RESET);
                                ...
                                System.out.println(
                                                VERDE +
                                                                "Detalle de ruta creado para la venta ID: " +
                                                                venta.getId() +
                                                                RESET);
                                ...
                                List<DetalleVentaResponseDTO> detalles = detalleVentaRep.findByVenta_Id(venta.getId());
                                rutaRes.setProductos(detalles);
        ...
        private Integer createRutaDetalleVenta(
                        Ruta nuevaRuta,
                        VentaPostDTO ventaDTO,
                        Venta venta) {
        ...
        private ProductoDimencionesDTO getTotalKgAndVolumen(
                        VentaPostDTO ventaDTO,
                        Venta venta) {
                ProductoDimencionesDTO dim = new ProductoDimencionesDTO(
                                BigDecimal.ZERO,
                                BigDecimal.ZERO);
        ...
        private double calcularDistancia(
                ...
                double latDistance = Math.toRadians(
                                lat2.doubleValue() - lat1.doubleValue());
                double lonDistance = Math.toRadians(
                                lon2.doubleValue() - lon1.doubleValue());
